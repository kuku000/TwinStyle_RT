...
def build_model(model_name, num_classes):
    ...
    elif model_name == 'SDABNet':
        return dabnet.DABNet(classes = 1)    
    else:
        return enet.ENet(1)
...
def create_seg_model(choose):
    if choose == "DABNet":
        model = build_model("DABNet", num_classes=19)
        model.eval()
        model = model.cuda()
        print("DABNet")
        model.load_state_dict(torch.load("/home/ubuntu/content/pretrained/DABNet_cityscapes.pth")["model"])
    elif choose=="SDABNet":
        model = build_model("SDABNet", num_classes=1)
        model.eval()
        model = model.cuda()
        print("SDABNet")
        model.load_state_dict(torch.load("/home/ubuntu/content/pretrained/best-model.pt"))
    else:
        model = build_model("ENet", num_classes=1)
        model.eval()
        model = model.cuda()
        print("ENet")
        model.load_state_dict(torch.load("/home/ubuntu/content/pretrained/ENet_bs16.pt"))
...
def create_style_model(style_number=0):
    model_file = glob.glob('/home/ubuntu/content/styles/*.pth')[style_number]
...
def load_image_style(frame, size=None, scale=None):
    img = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB).astype('uint8'))
...
def load_image(fname, model_name):
    if model_name =="ENet" or model_name == "SDABNet":
        image = np.asarray(fname, np.float32)
...
@torch.no_grad()
def get_masked_image(model, image, category, bg=0, model_name = "ENet"):
    if model_name == "ENet" :
        new=image.copy()
        new=new[None]
        
        mask = torch.Tensor(image.copy())/255
        
        device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        mask=mask.to(device)
        start_time = time.time()
        logits_mask = model(mask.unsqueeze(0))
        print(f"{model_name}:"+str(time.time()-start_time))
        #print('logits_mask shape:',logits_mask.shape)
        pred_mask=torch.sigmoid(logits_mask)
        pred_mask=(pred_mask>0.43)*1.0
        pr_np=pred_mask.detach().cpu().squeeze()
        #print('pr_np shape:',pr_np.shape)
        bin_mask=np.array(pr_np)
        #print('bin_mask shape:',bin_mask.shape)
        if bg:
            bin_mask = 1 - bin_mask

        image_original = image
        masked = bin_mask[:, :, None] * image_original.transpose(1,2,0)
        #print('pr_np shape:',pr_np.shape)
        #print('type',type(pr_np))
        #pr_np_umat = cv2.UMat(pr_np)
        out_pic=np.array(pr_np)
        # print('out picture shape:',out_pic.shape)
        # cv2.imshow('mask',out_pic)
        pic_masked=masked
        # print('pr_np shape:',pic_masked.shape)
        ...
        start_time = time.time()
        output = model(input_var)
        print("DABNet:"+str(time.time()-start_time))
        torch.cuda.synchronize()
        output = output.cpu().data[0].numpy()
        #print('output shape 1 ',output.shape)
        output = output.transpose(1, 2, 0)
        #print('output shape transpose ',output.shape)
        output = np.asarray(np.argmax(output, axis=2), dtype=np.uint8)
        ...
        #print('bin_mask[:, :, None] shape ',bin_mask[:, :, None].shape)
